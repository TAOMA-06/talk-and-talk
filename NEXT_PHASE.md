...
| 多支付渠道 | 支付宝等；仅微信路径存在 |
| 微信生产预支付完整化 | `RealWeChatPayProvider` 预支付/平台证书验签/回调解密仍为壳时不可真实收款 |
| 真实短信 | Aliyun / Tencent 等；当前仅 `mock` / `none`，生产禁止 mock |
...
| 项 | 说明 |
|----|------|
| CI | 无 GitHub Actions；本地 `npm test` / e2e / xcodebuild |
| OpenAPI 装饰器化 | 当前为手写 `packages/contracts/openapi/v1.yaml` |
| Metrics 鉴权 | `/api/v1/metrics` 需内网隔离 hardening |
| Redis ACL / 密码强制 | 文档已要求，compose 默认可无密码 |
| 微信证书 Docker secret 自动化 | 路径示例有，compose 未默认挂载 |
...
- Auth（手机 mock / Apple）、companions、订单 mock 支付、c1–c3 聊天审核、通知、Admin
- iOS 登录门控与核心演示链路
