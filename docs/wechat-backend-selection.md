...
| 最小实例 | staging 可为 0；production 建议至少 1，避免登录/支付冷启动 |
...
若云托管绑定 VPC 后关闭默认公网出向，需要配置 NAT 网关。否则后端无法访问 `api.weixin.qq.com`、微信支付 API、Apple JWKS 或内容审核提供商。
...
6. 检查云托管日志中没有 AppSecret、session_key、JWT、手机号或支付私钥泄露。
7. production 先灰度发布；健康检查和关键链路通过后再切换 100% 流量。
8. 切流前执行 `./backend/api/scripts/production-smoke.sh https://api.talkandtalk.app`，不得接受 degraded 依赖或 Mock/Disabled 支付 provider。
