# Talk & Talk 市场化调整计划（微信小程序 / 技术与商业审查版）

> **历史计划快照（2026-07-25，不得用于当前发布决策）。** 本文保留当时的市场研究、旧提交基线、阶段计划和自动化数字；后文的“当前状态”“待开发”“当前机器”均只代表该次审查。当前事实与放行结论以 [商用界面闭环台账](./commercial-interface-closure.md)、[商用市场交叉审查](./commercial-market-cross-audit-2026-08-01.md) 和 [生产检查清单](./production-checklist.md) 为准。
>
> 研究与审查日期：2026-07-25  
> 代码基线：`main` / `5408f17`（本轮改动仍在工作区，未代替负责人提交或发布）
...
| D4 功能范围 | **调整** | 当前发行只验收微信小程序 + NestJS。保留订单、支付、履约、安全与售后能力；删除会造成虚假价格/可约承诺的旧商用兜底和无入口死代码。社区与历史 iOS 不扩张，未来分别立项。 |
...
| 可售商品读模型、经营漏斗、真实信任指标 | **调整** | 代码、契约与自动化测试已补齐；上线前仍须用 staging 真实数据校准迁移、并发容量、查询延迟和漏斗口径。 |
...
| 实时语音 | 订单参与者、服务窗口、短效凭证、关房与争议事实的代码闭环已完成 | 保持默认关闭；仅在主体类目、CAM、可信回调、两台真机与弱网演练全部留证后启用 | **上线门禁** |
| 语音留言/媒体 | 生产存储和内容分析适配器未完成 | 保留现有能力；如扩大生产媒体交付，先补齐加密存储、审核和删除闭环 | **暂停** |
| 公共评价 | 已购评价、失败重试、分页与注销后的评分重算已进入正式路径 | 真实流量下继续核对误删、申诉与展示质量 | **保留** |
| 社区广场 | 增加冷启动与审核负担 | 代码和现有入口不动；如需调整发布范围，未来单独评审 | **保留** |
| 个性化推荐 | 偏好/排除、真实 SKU + 容量、可解释原因、归因指标与受控运营位已闭环 | 只在完成率、退款和安全指标证明需要时再评审更复杂模型 | **保留** |
| 收藏 | 对复购有轻价值 | 保留现有能力 | **保留** |
| 最近浏览、可约提醒 | 最近浏览可清除；可约提醒已有一次性授权、分阶段 claim/lease、终态核销和关闭保护 | 默认关闭提醒外部通道；取得微信模板与真实发送证据后再放量 | **上线门禁** |
| iOS、Apple 登录、短信、App 支付 | 当前后端部署目标为微信小程序 | 现有代码和接口保留；iOS 仅可在未来单独评审发布范围 | **保留** |
| 运营后台、审核、退款、审计 | 后端能力较完整 | 补齐真实运营 SOP 与权限验收 | **调整** |
| 产品数据 | 付费漏斗、推荐归因、供给/履约/退款与关键异步队列指标已进入运营面 | staging 校准口径、阈值和告警责任人 | **调整** |
...
- 公开列表、显式排序和推荐现在共用同一个可售读模型。
- 默认只返回未来 7 天内同时有匹配的当前上架商品、结构化时段和剩余容量的已发布/已核验对象。
- TRTC 未启用或紧急停止时，默认可售读模型只保留文字商品；详情与下单仍会再次校验。
...
没有时段的资料仍可在本人书签或旧链接中以“不可售”状态读取，但不会混入默认立即可约结果。

状态：**调整**（代码、单元测试和小程序冒烟已完成；真实数据量与延迟仍需 staging 验证）

### 3.5 页面价格和真实成交价格可能不是同一个来源

**代码事实**

- 发现、社区相关和订单后续推荐现在显示当前可售 SKU 的起价、起价商品时长、可选方式和下一可约时间；价格排序也使用 SKU 起价。
- 私人书签和最近浏览不再展示旧资料价，因为它们是回看入口，不是当前在售承诺。
- 商用模式新单强制 `serviceOfferingId`、`availabilityWindowId` 和 `clientRequestId`，不再退回旧资料价。
- 小程序在商品目录或结构化时段不可用时失败关闭，不会创建旧版资料价订单。
...
状态：**调整**（旧 `pricePerHalfHour` 仅保留内部/历史兼容；真实环境仍需核对卡片、支付和订单快照）
...
- 创建/更新 DTO 已拒绝直接写入评分、评价数、完成单数和响应时间。
- 服务完成会在同一事务累加完单数；陪伴者接单会基于已确认订单事实更新平均响应文案。
- 新迁移会从评价、完成订单和接单时间重算历史值。距离、在线状态仍是可编辑的展示/运营字段，不能被解释成平台核验的履约成果。
...
状态：**调整**（四项核心信任指标已改为派生；迁移必须先在 staging 对账，准时率/复购率仍待最小样本设计）
...
- 小程序完成订单已经同时提供可选公开已购评价、私密体验反馈/售后和按当前商品与时段重新预约。
- 公共评价与私密体验反馈使用不同数据表和权限边界；私密低分、标签或文字不会暴露给陪伴者或自动改变资金状态。
...
状态：**调整**（基础闭环已完成；仍需优化“已评价”状态和评价后的轻量反馈体验）

### 3.8 现有数据不能回答“为什么没成交”

**代码事实**

- 推荐模块现能按位置/陪伴者聚合曝光、查看、点击、接单、支付、开始、完成、评价、退款与归因 GMV。
- 商业后台新增以订单创建时间为 cohort 的请求、接单、支付、准时开始、完成、评价、退款、复购与净实收漏斗，不返回客户身份或聊天内容。
- 仍没有覆盖同意 → 首页 → 商品 → 时段全部前置步骤和完整渠道来源的统一客户端事件模型。
...
状态：**部分完成**（交易事实漏斗可用；前置客户端漏斗、完整渠道归因和自动周报仍待立项）
...
| C02 | 建立“可售商品列表”读模型 | **调整** | 代码已完成：默认目录和推荐只返回有效商品 + 未来 7 天真实剩余容量；staging 仍需验证真实数据与延迟。 |
| C03 | 统一公开价格来源 | **调整** | 卡片、详情、订单和支付以当前商品/订单快照为事实；价格策略与促销实验仍按 D7 暂停。 |
...
| C08 | 重做首页和详情 API | **调整** | 公开目录和推荐返回当前商品摘要与下一可约提示，详情及下单再次校验。 |
| C09 | 增加已购评价闭环 | **调整** | 仅完成订单可公开评价；私密体验反馈与公开评价分表、分权限。 |
| C10 | 建立全漏斗事件与 outbox | **待验证** | 已有订单 cohort 聚合漏斗与事务通知 outbox；跨 90 天可重放事件治理/数仓仍属于规模化阶段。 |
...
| C16 | 派生真实信任指标 | **调整** | 评分、评价数、完成单和响应时间已禁止手填并按事实派生；历史校准迁移须 staging 对账，准时率仍待最小样本设计。 |
...
| 检查 | 结果 |
|---|---|
| 后端单元测试 | **通过：75/75 suites、567/567 tests**（新增覆盖可售容量推荐门禁、商用结构化下单、派生信任指标、不可变接单口径与商业漏斗；既有语音、支付、订单与聊天权益回归继续通过） |
| 后端生产构建与产物检查 | **通过** |
| Prisma schema / 语音迁移结构 | **通过：`npx prisma validate`** |
| OpenAPI YAML | **通过：可解析且保留语音房间契约** |
| 小程序结构校验 | **通过：14 pages，5 tabs** |
| 小程序运行时 smoke | **通过：覆盖 373 个 API 调用分支；已删除详情页的旧价格/自由时间商用兜底，验证发现/社区/订单推荐共用当前 SKU 展示，并继续覆盖支付、语音关房、完成订单只读和后台凭证竞态退出** |
...
## 16. 技术与商务负责人已作出的 8 个决定

以下是本轮执行口径；外部主体、资质、真实支付和生产发布仍须由有权限的负责人签字，不因这张表自动放行。

- [x] **D1 定位：**首发定位为非医疗、按时、付费、有边界的真人陪伴。
- [x] **D2 商品：**现在补齐实时语音；只能作为订单内、服务窗口内的真实语音服务。
- [x] **D3 交易：**陪伴者必须手动接单或拒单；接受后客户支付，陪伴者再手动开始服务。
- [x] **D4 范围：**当前只验收微信小程序 + NestJS；保留核心交易/安全能力，删除会造成虚假承诺的旧商用兜底，社区与历史 iOS 不扩张。
- [x] **D5 微信架构：**小程序做前端，NestJS 部署 CloudBase 云托管，PostgreSQL/Redis 同 VPC；真实环境仍按上线门禁验收。
- [x] **D6 首发规模：**先做不超过 20 名已核验供给、累计 100 笔已支付订单的受控试运营；达到准时、完成、退款、对账和供给密度门槛后再放量。
...
7. [x] 可售读模型统一默认目录与全部推荐入口；卡片使用当前 SKU，私人回看入口不承诺旧资料价；
8. [x] 商用新单强制幂等键、有效商品和结构化时段；目录或容量故障失败关闭；
9. [x] 评分、评价数、完单数和响应时间改为只读派生，并提供历史数据校准迁移；
10. [x] 商业订单 cohort 漏斗和推荐归因使用统一接单事实，返回履约、退款、复购与净实收聚合，不返回客户身份或聊天内容；
11. [x] 签名/RTC 依赖锁定、微信开发者工具 NPM 构建、配置校验、迁移、OpenAPI、部署核对表和自动化测试；
