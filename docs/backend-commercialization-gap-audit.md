...
Prometheus 与推荐曝光事件不等于业务经营驾驶舱。建议只读聚合（不碰支付权威写路径）：

| 指标组 | 示例 |
|---|---|
| 供给漏斗 | 入驻→培训完成→上架→有可约容量→首次接单 |
| 履约 | 确认率、拒单率、响应超时、服务开始准时率、完成率 |
| 时段 | 放出容量、占用率、空档浪费、改期接受率 |
| 资金售后 | 退款率/原因、投诉 SLA、追偿开放龄期 |
| 复购 | 同陪伴者再次下单、书签转化、可约提醒投递成功率（聚合） |
| 安全 | 审核积压、申诉超时、危机资源查看（无个人明细外泄） |

消费方：`/admin/commercial/*`；禁止把聊天正文、证件、openid 带入看板。

### 4.2 轻量服务质量分级与整改任务

壹心理倾诉公开投诉规则含 A–E 分级、督导反思、实习期、隐藏主页等。Talk&Talk **不复制医疗伦理体系**，但应有可运营的质量层：

| 能力 | 设计要点 |
|---|---|
| 质量案件 | 与资金退款、内容审核分域；可关联订单/会话证据引用 |
| 轻量等级 | 例如：未见问题 / 需整改 / 限制接单 / 下架合作（非临床胜任力宣称） |
| 整改任务 | 培训重做、反思材料引用、到期未完成则自动限制上架 |
| 考核计入 | 有效投诉计数进入既有 companion quality 指标，不公开给客户细节 |
| 复用 | `companion-lifecycle`、账号动作、非自审复核、独立 `/review/` 边界 |

### 4.3 可约提醒与候补

- 可约提醒投递 runner 默认关闭；放量需模板审批 + staging 真链 + 聚合指标值班（已有文档边界）
- “指定时段候补队列”为可选产品：需独立模型（候补、失效、容量重检、一次性通知），确认产品需要后再做
...
| 媒体存储 | 生产 COS/S3 类适配器；reserve→upload→verify→delete；审核失败与到期删除进入 readiness |
...
若进入实现，按独立项目推进，避免与商用放行证据包互相阻塞：

1. **资金自动化**：付款提供方适配 + 提现状态机 + 回单/税务字段 + readiness
2. **经营指标**：只读聚合 API + admin 消费
3. **质量整改**：轻量分级与整改任务，复用 companion-lifecycle
4. **完整表面**：TRTC/媒体，严格服从 `COMMERCIAL_SURFACE` 门禁

验收纪律不变：双人控制、失败关闭、readiness 可证明、不把外部 Mock 写成已商用。
