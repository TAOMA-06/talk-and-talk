import SwiftUI

@MainActor
public final class AppStore: ObservableObject {
    @Published public var user: User = MockData.user
    @Published public var companions: [Companion] = MockData.companions
    @Published public var themes: [Theme] = MockData.themes
    @Published public var orders: [Order] = MockData.orders
    @Published public var reviews: [Review] = MockData.reviews
    @Published public var messages: [Message] = MockData.messages
    
    @Published public var selectedTheme: Theme?
    @Published public var navigationPath = NavigationPath()
    
    public init() {}
    
    public func companions(for theme: Theme?) -> [Companion] {
        guard let theme = theme else { return companions }
        return companions.filter { $0.specialties.contains(theme.name) }
    }
    
    public func companion(by id: String) -> Companion? {
        companions.first { $0.id == id }
    }
    
    public func reviews(for companionId: String) -> [Review] {
        reviews.filter { $0.companionId == companionId }
    }
    
    public func messages(for companionId: String) -> [Message] {
        messages.filter { $0.senderId == companionId || $0.senderId == user.id }
    }
    
    public func sendMessage(_ content: String, to companionId: String) {
        let message = Message(
            id: UUID().uuidString,
            senderId: user.id,
            content: content,
            type: .text,
            timestamp: Date()
        )
        messages.append(message)
        
        // Simulate companion reply
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) { [weak self] in
            guard let self = self else { return }
            let reply = Message(
                id: UUID().uuidString,
                senderId: companionId,
                content: "我理解你的感受，继续说说吧，我在这里陪着你。",
                type: .text,
                timestamp: Date()
            )
            self.messages.append(reply)
        }
    }
    
    public func createOrder(companionId: String, themeId: String, duration: Double) -> Order {
        let companion = companion(by: companionId)!
        let order = Order(
            id: UUID().uuidString,
            companionId: companionId,
            themeId: themeId,
            duration: duration,
            totalPrice: Int(Double(companion.pricePerHour) * duration),
            status: .confirmed,
            createdAt: Date(),
            scheduledAt: Date()
        )
        orders.append(order)
        return order
    }
    
    public func verifyUser() {
        user = User(
            id: user.id,
            name: user.name,
            avatar: user.avatar,
            isVerified: true,
            phone: user.phone
        )
    }
}

enum MockData {
    static let user = User(
        id: "u1",
        name: "小美",
        avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=xiaomei",
        isVerified: false,
        phone: "138****8888"
    )
    
    static let themes: [Theme] = [
        Theme(id: "t1", name: "情感倾诉", icon: "heart.fill", description: "倾听你的心事，给予温暖陪伴"),
        Theme(id: "t2", name: "职场解压", icon: "briefcase.fill", description: "释放工作压力，找回内心平静"),
        Theme(id: "t3", name: "学习陪伴", icon: "book.fill", description: "专注学习时光，互相督促进步"),
        Theme(id: "t4", name: "运动鼓励", icon: "figure.run", description: "一起运动打卡，保持健康生活"),
        Theme(id: "t5", name: "旅行分享", icon: "airplane", description: "分享旅途故事，探索世界美好"),
        Theme(id: "t6", name: "美食探索", icon: "fork.knife", description: "发现美食乐趣，分享味蕾体验")
    ]
    
    static let companions: [Companion] = [
        Companion(
            id: "c1",
            name: "林悦",
            avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=linyue",
            tags: ["温柔倾听", "心理学背景", "深夜在线"],
            rating: 4.9,
            reviewCount: 128,
            pricePerHour: 88,
            isOnline: true,
            isVerified: true,
            bio: "国家二级心理咨询师，擅长情感疏导与压力管理。愿做你深夜里的那盏灯，倾听你的故事，陪你走过低谷。",
            availableTimes: ["09:00", "14:00", "20:00", "22:00"],
            languages: ["中文", "英语"],
            specialties: ["情感倾诉", "职场解压"]
        ),
        Companion(
            id: "c2",
            name: "苏晴",
            avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=suqing",
            tags: ["阳光活力", "健身教练", "早起型"],
            rating: 4.8,
            reviewCount: 96,
            pricePerHour: 68,
            isOnline: true,
            isVerified: true,
            bio: "健身教练兼营养师，热爱运动与生活。可以陪你一起制定运动计划，分享健康饮食心得，让每一天都充满活力！",
            availableTimes: ["06:00", "07:00", "18:00", "19:00"],
            languages: ["中文"],
            specialties: ["运动鼓励", "美食探索"]
        ),
        Companion(
            id: "c3",
            name: "陈墨",
            avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=chenmo",
            tags: ["知性优雅", "旅行达人", "摄影师"],
            rating: 4.7,
            reviewCount: 84,
            pricePerHour: 78,
            isOnline: false,
            isVerified: true,
            bio: "自由摄影师，足迹遍布30+国家。热爱分享旅行故事与摄影技巧，带你用镜头发现世界的美。",
            availableTimes: ["10:00", "15:00", "19:00"],
            languages: ["中文", "英语", "日语"],
            specialties: ["旅行分享", "学习陪伴"]
        ),
        Companion(
            id: "c4",
            name: "周暖",
            avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=zhounuan",
            tags: ["治愈系", "手绘爱好者", "慢生活"],
            rating: 4.9,
            reviewCount: 156,
            pricePerHour: 58,
            isOnline: true,
            isVerified: true,
            bio: "插画师，喜欢手绘与手工。享受慢生活的美好，愿意陪你一起画画、做手工，在安静中找到内心的宁静。",
            availableTimes: ["09:00", "13:00", "16:00", "20:00"],
            languages: ["中文"],
            specialties: ["学习陪伴", "情感倾诉"]
        ),
        Companion(
            id: "c5",
            name: "赵朗",
            avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=zhaolang",
            tags: ["幽默风趣", "程序员", "夜猫子"],
            rating: 4.6,
            reviewCount: 72,
            pricePerHour: 98,
            isOnline: true,
            isVerified: false,
            bio: "全栈工程师，热爱技术与创业。可以陪你聊职业规划、技术学习，或者只是吐槽一下工作中的那些事儿。",
            availableTimes: ["20:00", "21:00", "22:00", "23:00"],
            languages: ["中文", "英语"],
            specialties: ["职场解压", "学习陪伴"]
        ),
        Companion(
            id: "c6",
            name: "吴悠",
            avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=wuyou",
            tags: ["美食家", "烘焙达人", "生活家"],
            rating: 4.8,
            reviewCount: 112,
            pricePerHour: 72,
            isOnline: false,
            isVerified: true,
            bio: "美食博主，擅长烘焙与家常菜。可以分享独家食谱，陪你探讨美食文化，让每一餐都充满幸福感。",
            availableTimes: ["11:00", "14:00", "17:00", "20:00"],
            languages: ["中文", "粤语"],
            specialties: ["美食探索", "情感倾诉"]
        )
    ]
    
    static let orders: [Order] = [
        Order(
            id: "o1",
            companionId: "c1",
            themeId: "t1",
            duration: 1,
            totalPrice: 88,
            status: .completed,
            createdAt: Date(timeIntervalSince1970: 1705312800),
            scheduledAt: Date(timeIntervalSince1970: 1705356000)
        )
    ]
    
    static let reviews: [Review] = [
        Review(
            id: "r1",
            companionId: "c1",
            userName: "匿名用户",
            rating: 5,
            content: "林悦老师非常温柔专业，倾听了我工作上的困扰，给了很多实用的建议。聊完之后感觉轻松多了！",
            createdAt: Date(timeIntervalSince1970: 1705399200)
        ),
        Review(
            id: "r2",
            companionId: "c1",
            userName: "小雨",
            rating: 5,
            content: "第二次预约了，每次都能感受到真诚的关心。深夜有人倾听的感觉真的很好。",
            createdAt: Date(timeIntervalSince1970: 1705226400)
        ),
        Review(
            id: "r3",
            companionId: "c1",
            userName: "阿明",
            rating: 4,
            content: "整体不错，就是预约时间有点紧张，希望可以增加更多时段。",
            createdAt: Date(timeIntervalSince1970: 1705053600)
        )
    ]
    
    static let messages: [Message] = [
        Message(
            id: "m1",
            senderId: "system",
            content: "订单已确认，您可以在约定时间开始沟通。",
            type: .system,
            timestamp: Date(timeIntervalSince1970: 1705353300)
        ),
        Message(
            id: "m2",
            senderId: "c1",
            content: "你好呀，我是林悦。很高兴能陪伴你度过这段时间。有什么想聊的吗？",
            type: .text,
            timestamp: Date(timeIntervalSince1970: 1705356000)
        ),
        Message(
            id: "m3",
            senderId: "u1",
            content: "最近工作压力好大，感觉快撑不住了...",
            type: .text,
            timestamp: Date(timeIntervalSince1970: 1705356120)
        ),
        Message(
            id: "m4",
            senderId: "c1",
            content: "听起来你真的很辛苦。愿意多说说吗？是什么让你感到压力最大呢？",
            type: .text,
            timestamp: Date(timeIntervalSince1970: 1705356180)
        )
    ]
}
