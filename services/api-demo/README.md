...
本目录是 Talk&Talk 的 **本机聊天与内容安全审查后端**，同时提供 **Web 审核后台**（浏览器打开根路径即可）。

- **业务上做什么**：接收用户聊天文本 → 规则 + DeepSeek 判断是否违规（私联/线下/转账等）→ 拦截或放行 → 生成管理员审核工单  
- **技术上是什么**：Node 22 原生模块，零 npm 依赖；数据仅存内存，重启恢复演示数据  

**不清楚「AI 识别」用途的协作者，请先读仓库根目录 [GUIDE.md](../GUIDE.md) 第 2 节。**
